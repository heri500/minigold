(function ($, Drupal, once) {
  ...
  Drupal.behaviors.requestKemasanModal = {
    attach: function (context, settings) {
      ...
      const tableId = settings.dataRequestKemasan?.tableId || 'data-request-kemasan-table';
      ...
      function setupDataTableEvents() {
        ...
        function addDtControlClass() {
          ...
          $('#' + tableId + ' tbody tr').each(function () {
            ...
            var dataId = $tds.eq(drupalSettings.dataRequestKemasan.colIdIdx).text().trim();

            // Get data-status from <div> inside 10th column
            var dataStatus = $tds.eq(drupalSettings.dataRequestKemasan.colIdIdx + 4).find('div').data('status');
        ...
        function loadChildRowData(row, tr, rowId) {
          ...
          const detailUrl = baseUrl + 'data-request-kemasan/detailrequest/' + rowId;
      ...
      function attachEventHandlers() {
        $('.edit-icon').off('click.editRequesticon').on('click.editRequesticon', function(e) {
          ...
          let modalUrl = baseUrl + 'data-request-kemasan/add/' + requestId;
      ...
      $(document).on('click.deleteRequestIcon', '.delete-icon', function (e) {
        ...
        let baseUrl = drupalSettings.path.baseUrl || '/';
        let deleteUrl = baseUrl + 'data-request-kemasan/delete/' + idRequest;
        const deleteConfirmation = confirm('Yakin ingin menghapus request kemasan ini...??!');
