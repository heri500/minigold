...
namespace Drupal\data_request_kemasan\Controller;
...
/**
 * Returns responses for Data request kemasan routes.
 */
final class DataRequestKemasanController extends ControllerBase {
  /**
   * The form builder.
   *
  ...
  public function __invoke(): array {
    ...
    $ColIdIdx = 1;
    if ($this->currentUser->hasPermission('administer request kemasan')) {
      $header = [...$header,
        ['data' => '', 'class' => 'no-sort dt-orderable-none', 'data-orderable' => 'false', 'data-dt-order' => 'disable', 'data-searchable' => 'false'],
        ['data' => '', 'class' => 'no-sort dt-orderable-none', 'data-orderable' => 'false', 'data-dt-order' => 'disable', 'data-searchable' => 'false'],
      ];
    ...
    $table_id = 'data-request-kemasan-table';
    ...
    return [
      ...
      'table' => [
        ...
        '#attached' => [
          'library' => [
            'data_request_kemasan/datarequestkemasan_js',
            'core/drupal.dialog.ajax',// Define the library in the module's *.libraries.yml file.
          ],
          'drupalSettings' => [
            'dataRequestKemasan' => [
              'modalFormUrl' => Url::fromRoute('data_request_kemasan.modal_form', ['id' => 0])->toString(),
              'tableId' => $table_id,
              'colIdIdx' => $ColIdIdx,
            ],
          ],
  ...
  private function getDataTableOptions() {
    if ($this->currentUser->hasPermission('administer request kemasan')) {
      $AjaxUrl = base_path() . 'datasource/getdata/request_kemasan?editable=1&deletable=1&hasdetail=1';
      $orderedColumn = 3;
    }else{
      $AjaxUrl = base_path() . 'datasource/getdata/request_kemasan?hasdetail=1';
      $orderedColumn = 0;
    }
    return [
      'info' => TRUE,
  ...
  public function modalForm($id = 0) {
    ...
    // Build the form
    $form = $this->formBuilder->getForm('Drupal\data_request_kemasan\Form\EditRequestKemasan', $id);

    // Add the form to a modal dialog
    $title = ($id > 0) ? $this->t('Edit Request Kemasan') : $this->t('Add Request Kemasan');
  ...
  public function detailRequest($id) {
    $data = [];
    if (!empty($id)) {
      $table_name = 'request_kemasan_detail';
      $field_data = $this->dataSourceService->getTableFields($table_name);
      $field_value[] = ['id_request_kemasan' => $id];
      $left_join = [
        'alias' => 'p',
        'table_name' => 'product',
  ...
  public function deleteRequestKemasan($id = NULL) {
    if (!empty($id)) {
      $fieldsid_data = ['field' => 'id_request_kemasan', 'value' => $id];
      // Delete all existing detail records for this request
      $query = $this->dataSourceService->fetchRecordsById('request_kemasan', ['id_request_kemasan'], $id);
      if (!empty($query)) {
        // Delete all existing detail records for this request
        $this->dataSourceService->deleteTableById('request_kemasan_detail',$fieldsid_data);

        // Get Request Admin ID related to Request Produksi
        $query = $this->dataSourceService->fetchRecordsById('request_admin_kemasan', ['id_request_admin'], $id);
        if (!empty($query)){
          $RequestAdminId = $query->id_request_admin;
          // Delete the main request
          $this->dataSourceService->deleteTableById('request_admin_kemasan',$fieldsid_data);
        ...
        $this->dataSourceService->deleteTableById('request_kemasan',$fieldsid_data);
      }
