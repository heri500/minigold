...
gpos_theme theme uses [Webpack](https://webpack.js.org) to compile and
bundle SASS and JS.
...
Go to the root of gpos_theme theme and run the following commands:
`nvm use`
...
`drush en gpos_theme -y`
`drush config-set system.theme default gpos_theme -y`
